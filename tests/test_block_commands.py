...
"""Tests for harness_guards/block_commands.py."""
...
from harness_guards import block_commands
...
class TestExtractCommand:
    """Codex names its exec tool differently and may hand over an argv list."""

    def test_string_command(self) -> None:
        assert block_commands.extract_command({"command": "git push"}) == "git push"

    def test_cmd_key(self) -> None:
        assert block_commands.extract_command({"cmd": "git push"}) == "git push"

    def test_argv_list_is_joined_unquoted(self) -> None:
        argv = ["bash", "-lc", "cd /tmp && git push"]
        assert (
            block_commands.extract_command({"command": argv})
            == "bash -lc cd /tmp && git push"
        )

    def test_non_string_list_ignored(self) -> None:
        assert block_commands.extract_command({"command": [1, 2]}) == ""

    def test_non_dict(self) -> None:
        assert block_commands.extract_command("git push") == ""

    def test_input_key_is_not_a_command(self) -> None:
        """An apply_patch body is not a command, however it reads."""
        assert block_commands.extract_command({"input": "sudo rm -rf /"}) == ""


class TestCodexShapedPayloads:
    def test_argv_list_blocked(self, monkeypatch: pytest.MonkeyPatch) -> None:
        data = {
    ...
    def test_chained_command_inside_argv_list_blocked(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """shlex.join would hide this behind quotes; a space join does not."""
        data = {
            "tool_name": "unified_exec",
            "tool_input": {"command": ["bash", "-lc", "make build && sudo install"]},
        }
        monkeypatch.setattr(sys, "stdin", io.StringIO(json.dumps(data)))
        with pytest.raises(SystemExit) as exc_info:
            block_commands.main()
    ...
    def test_cmd_key_blocked(self, monkeypatch: pytest.MonkeyPatch) -> None:
        data = {"tool_name": "exec_command", "tool_input": {"cmd": "sudo reboot"}}
        monkeypatch.setattr(sys, "stdin", io.StringIO(json.dumps(data)))
        with pytest.raises(SystemExit) as exc_info:
            block_commands.main()
        assert exc_info.value.code == 2

    def test_patch_body_not_treated_as_command(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        data = {
