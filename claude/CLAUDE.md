...
## Restrictions
- **Never execute**: `git push`, `git pull`, `git add`, `sudo`, or `su`
...
## Attribution
- **Model is the raw id the harness reports, never a friendly name** — `claude-opus-5`, not `Claude Opus 5`. A friendly-name table is wrong on every model release and nothing detects when it goes stale. Applies to both forms below
- **Commits**: include `Assisted-by: <Tool> (<Model>)` (e.g., `Assisted-by: Claude Code (claude-opus-5)`). The commit skill's hook writes these from the transcript, so a commit can carry several lines when several models authored the change — that is correct, not duplication to clean up
- **New files**: include `Generated By: <Tool> (<Model>)` as comment in file header (e.g., `Generated By: Claude Code (claude-opus-5)`)
